...
def load_and_collate_data(input_dir):
    ...
    for my_file_pair in only_files:
        df = pd.read_csv(my_file_pair[0],
                         sep = '\t',
                         names = ['Gene', my_file_pair[1]]
                        )
...
def save_data(output_dir, merged_data):
    merged_data.to_csv(os.path.join(output_dir, 'AllCounts_Blueberry.tsv'),
                       sep='\t',
                       index = False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('input_dir', type=str,
                        help='parent directory of input counts')
    parser.add_argument('--output_dir', '-o', type=str,
                        default=os.path.join(path_main,'../../../Input/Count/Collate/'),
                        help='parent directory of input counts')
    parser.add_argument('-v', '--verbose',
                        action='store_true',
                        help='set debgging level to DEBUG')
